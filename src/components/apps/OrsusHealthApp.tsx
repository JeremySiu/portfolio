...
const ORSUS_URL = 'https://orsus-health.vercel.app'
export const ORSUS_HOST = 'orsus-health.vercel.app'
const ORSUS_RELOAD_EVENT = 'orsus:reload'
...
export default function OrsusHealthApp({ onClose }: { onClose?: () => void }) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      window.open(ORSUS_URL, '_blank', 'noopener,noreferrer')
      onClose?.()
    }, LOAD_TIMEOUT_MS)
  ...
  useEffect(() => {
    ...
    window.addEventListener(ORSUS_RELOAD_EVENT, onReload)
    return () => window.removeEventListener(ORSUS_RELOAD_EVENT, onReload)
  }, [])
        ...
        >
          <div
            style={{
              ...
              height: 34,
              borderRadius: '50%',
              border: '3px solid #d1e9f0',
              borderTopColor: '#0ea5e9',
              animation: 'orsus-spin 0.8s linear infinite',
            }}
          />
          <span style={{ fontSize: 14, fontWeight: 500 }}>Loading Orsus Health…</span>
          <style>{`@keyframes orsus-spin { to { transform: rotate(360deg); } }`}</style>
        </div>
      )}
    </div>
  )
...
export function SafariReloadButton({ tone = 'onLight' }: { tone?: 'onLight' | 'onDark' }) {
  ...
  const reload = () => window.dispatchEvent(new Event(ORSUS_RELOAD_EVENT))
  return (
    <button
      type="button"
